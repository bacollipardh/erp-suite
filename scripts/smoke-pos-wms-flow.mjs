...
const itemCode = process.env.ERP_POS_ITEM_CODE || 'USB-CABLE-001';
const qty = Number(process.env.ERP_POS_QTY || 2);
...
async function main() {
  ...
  const invoice = await fetchJson(
    'sales-invoices',
    {
      ...
      body: JSON.stringify({
        ...
        notes: 'Smoke POS -> WMS -> posted invoice',
        lines: [
          {
            itemId: item.id,
            qty,
  ...
  await fetchJson(
    `wms/picking/sales-invoices/${invoice.id}/plan`,
    { method: 'POST', headers, body: JSON.stringify({}) },
    'plan WMS pick',
  );
  await fetchJson(
    `wms/picking/sales-invoices/${invoice.id}/confirm`,
    { method: 'POST', headers, body: JSON.stringify({}) },
    'confirm WMS pick',
  );
  await fetchJson(
    `wms/packing/sales-invoices/${invoice.id}/pack`,
    { method: 'POST', headers, body: JSON.stringify({}) },
    'pack WMS invoice',
  );
  ...
  console.log('Smoke POS -> WMS -> posted invoice passed.');
}
