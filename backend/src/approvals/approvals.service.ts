...
type FindRequestsQuery = { status?: string; scope?: string; search?: string; limit?: string };
  ...
  async getPendingBadge(userId: string) {
    const rows = await this.prisma.$queryRawUnsafe<any[]>(
      ...
      SELECT
        ...
        COUNT(*) FILTER (WHERE r.status = 'PENDING' AND EXTRACT(EPOCH FROM (now() - r.requested_at)) / 3600 > $2)::int AS overdue_all,
    ...
    return { pendingAll: Number(rows[0]?.pending_all ?? 0), pendingForMe: Number(rows[0]?.pending_for_me ?? 0), overdueAll: Number(rows[0]?.overdue_all ?? 0), slaHours: SLA_HOURS };
  }
