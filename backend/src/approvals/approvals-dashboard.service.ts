    ...
    const [summaryRows, byEntityRows, byStatusRows, agingRows, recentRows, topPendingRows, eventRows] = await Promise.all([
      this.prisma.$queryRawUnsafe<any[]>(`
        SELECT
          ...
          COUNT(*) FILTER (WHERE status = 'PENDING' AND EXTRACT(EPOCH FROM (now() - requested_at)) / 3600 > COALESCE((SELECT sla_hours FROM approval_policies ap WHERE ap.id = policy_id LIMIT 1), 24))::int AS overdue_count,
