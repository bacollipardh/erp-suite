...
export class ControlTowerService {
  ...
  async getExceptions() {
    ...
    const bySeverity = this.groupCount(items, 'severity');
    const byCategory = this.groupCount(items, 'category');
    const exposureAmount = round2(items.reduce((sum, item) => sum + Number(item.amount ?? 0), 0));
    ...
    return {
      summary: {
        ...
        exposureAmount,
        financeCount: byCategory.FINANCE ?? 0,
        collectionsCount: byCategory.COLLECTIONS ?? 0,
        payablesCount: byCategory.PAYABLES ?? 0,
        stockCount: byCategory.STOCK ?? 0,
        controlCount: byCategory.CONTROL ?? 0,
      },
      bySeverity,
      byCategory,
      items,
  ...
  private async getStockAlerts() {
    const rows = await this.prisma.$queryRawUnsafe<any[]>(`
      SELECT
        ...
        GREATEST(0, 5 - sb.qty_on_hand)::numeric AS amount,
        NULL::int AS days_overdue,
