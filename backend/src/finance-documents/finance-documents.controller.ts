...
export class FinanceDocumentsController {
  ...
  @RequirePermissions(PERMISSIONS.salesInvoicesPay)
  createCustomerReceipt(@Body() dto: CreateCustomerReceiptDto, @CurrentUser() user: JwtPayload) {
  ...
  @RequirePermissions(PERMISSIONS.purchaseInvoicesPay)
  createSupplierPayment(@Body() dto: CreateSupplierPaymentDto, @CurrentUser() user: JwtPayload) {
