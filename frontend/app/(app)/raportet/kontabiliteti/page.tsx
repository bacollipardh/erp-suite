...
export default async function AccountingReportsPage({
  ...
  const [trialBalance, profitLoss, balanceSheet, vatLedger] = await Promise.all([
    ...
    api.query<VatLedgerResponse>('accounting/vat-ledger', {
      dateFrom,
      dateTo,
      side: vatSide,
      search: vatSearch,
  ...
  return (
    <div className="space-y-5">
      ...
      <form className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-3 xl:grid-cols-6">
          ...
          <input
            type="text"
            name="vatSearch"
            defaultValue={vatSearch}
            placeholder="Kerko dokument ose pale"
            className="rounded-lg border border-slate-300 px-3 py-2 text-sm"
          />
        </div>
      ...
      <div className="grid grid-cols-2 gap-4 xl:grid-cols-6">
        ...
        <MetricCard
          label="VAT Output"
          value={fmtMoney(vatLedger.summary.outputVat)}
          sub={fmtMoney(vatLedger.summary.outputTaxableBase)}
        />
        <MetricCard
          label="VAT Input"
          value={fmtMoney(vatLedger.summary.inputVat)}
          sub={fmtMoney(vatLedger.summary.inputTaxableBase)}
        />
      </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="border-b border-slate-200 px-5 py-4">
          <h2 className="text-base font-semibold text-slate-900">VAT / Tax Ledger</h2>
          <p className="mt-1 text-sm text-slate-500">
            Regjistri i TVSH-se nga faturat e shitjes, blerjes, kthimet dhe manual VAT adjustments.
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 border-b border-slate-100 px-5 py-4 xl:grid-cols-4">
          <MetricCard
            ...
            value={fmtMoney(vatLedger.summary.netVatPayable)}
            sub={`${vatLedger.summary.documentCount} regjistrime`}
          />
          <MetricCard
            label="Manual Adjustments"
            value={vatLedger.summary.manualAdjustmentCount}
            sub="Journal entries VAT"
          />
          <MetricCard
            label="Output Base"
            value={fmtMoney(vatLedger.summary.outputTaxableBase)}
            sub={fmtMoney(vatLedger.summary.outputVat)}
          />
          <MetricCard
            label="Input Base"
            value={fmtMoney(vatLedger.summary.inputTaxableBase)}
            sub={fmtMoney(vatLedger.summary.inputVat)}
          />
        </div>
              ...
              {vatLedger.items.length === 0 ? (
                <tr>
                  <td colSpan={6} className="px-4 py-10 text-center text-sm text-slate-400">
                    Nuk ka VAT ledger rows per filtrat e zgjedhur.
                  </td>
                </tr>
              ) : null}
            </tbody>
