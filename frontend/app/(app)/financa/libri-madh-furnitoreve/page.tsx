...
function sourceTypeHref(sourceType: string, sourceId: string) {
  if (sourceType === 'purchase-invoices') return `/purchase-invoices/${sourceId}`;
  if (sourceType === 'supplier-payments') return `/pagesat/${sourceId}`;
  return null;
}
...
export default async function Page({ searchParams }: { searchParams?: Promise<SearchParams> }) {
  ...
  const supplierId = value(params, 'supplierId');
  const dateFrom = value(params, 'dateFrom');
  const dateTo = value(params, 'dateTo');

  const [suppliers, ledger] = await Promise.all([
    api.list<Party>('suppliers', { limit: 500, sortBy: 'name', sortOrder: 'asc' }),
    api.query<LedgerResponse>('statements/suppliers/ledger', {
      supplierId,
      dateFrom,
      dateTo,
      limit: 2000,
    ...
    <div className="space-y-5">
      ...
      <form className="rounded-xl border bg-white p-4 shadow-sm grid gap-3 md:grid-cols-4">
        <label className="space-y-1 text-sm md:col-span-2">
          <span className="font-medium text-slate-700">Furnitori</span>
          <select
            name="supplierId"
      ...
      <div className="grid gap-3 md:grid-cols-5">
        ...
        <div className="rounded-xl border bg-white p-4 shadow-sm">
          <div className="text-xs text-slate-500 mb-1">Total Pagesa</div>
          <div className="text-lg font-semibold text-emerald-700">{money(ledger.totalDebit)}</div>
        </div>
        <div className="rounded-xl border bg-white p-4 shadow-sm">
          <div className="text-xs text-slate-500 mb-1">Total Fatura</div>
          <div className="text-lg font-semibold text-blue-700">{money(ledger.totalCredit)}</div>
        </div>
        <div className="rounded-xl border bg-white p-4 shadow-sm">
        ...
        <div className="rounded-xl border bg-white p-4 shadow-sm">
          <div className="text-xs text-slate-500 mb-1">Pa Paguar</div>
          <div
      ...
      {ledger.totalOutstanding > 0 && (
        <div className="rounded-xl border bg-white p-4 shadow-sm">
          <h3 className="text-sm font-semibold text-slate-700 mb-3">Analiza e Plakjes (Detyrimet e Pashlyera ndaj Furnitorëve)</h3>
          <div className="grid gap-3 md:grid-cols-5">
