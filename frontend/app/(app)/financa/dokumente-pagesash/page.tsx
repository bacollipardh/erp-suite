...
export default async function Page() {
  ...
  const payments = await api.list<Payment>('supplier-payments');

  return (
    <div className="space-y-5">
      <PageHeader
        title="Dokumente Pagesash"
        description="Dokumente formale financiare per pagesat cash/bank ndaj furnitoreve dhe avancat."
        createHref="/financa/dokumente-pagesash/new"
        createLabel="Krijo Dokument Pagese"
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm">
            <thead className="bg-slate-50 border-b">
              <tr>
                {['Dokumenti', 'Data', 'Furnitori', 'Llogaria', 'Shuma', 'Aplikuar', 'Pa aplikuar', 'Statusi', 'Veprim'].map((title) => (
            ...
            <tbody className="divide-y divide-slate-100">
              {payments.length === 0 ? (
                <tr><td colSpan={9} className="px-4 py-12 text-center text-sm text-slate-400">Nuk ka dokument pagese te regjistruar.</td></tr>
              ) : payments.map((row) => (
                <tr key={row.id} className="hover:bg-slate-50/70 transition-colors">
                  ...
                  <td className="px-3 py-2 text-slate-700 whitespace-nowrap">{row.supplier?.name ?? '-'}</td>
                  ...
                  <td className="px-3 py-2 pr-4"><FinanceDocumentPostButton endpoint="supplier-payments" id={row.id} disabled={row.status !== 'DRAFT'} /></td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div className="rounded-lg bg-amber-50 border border-amber-200 px-3 py-2 text-xs text-amber-800">
        Faza 1: dokumenti pa alokim postohet si unapplied payment / supplier advance. Alokimi direkt ne fatura shtohet ne fazen tjeter.
      </div>
    </div>
  );
}
