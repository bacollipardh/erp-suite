...
export default async function Page({ searchParams }: { searchParams?: Promise<SearchParams> }) {
  ...
  const [suppliers, statement] = await Promise.all([
    api.list<Party>('suppliers', { limit: 500, sortBy: 'name', sortOrder: 'asc' }),
    api.query<StatementResponse>('statements/suppliers', {
      supplierId,
      dateFrom,
      dateTo,
      limit: 1000,
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        ...
        description="Kartela e furnitorit me fatura blerje, pagesa dhe running payable balance."
      />
      ...
      <form className="rounded-xl border bg-white p-4 shadow-sm grid gap-3 md:grid-cols-4">
        <label className="space-y-1 text-sm md:col-span-2">
          ...
          <select name="supplierId" defaultValue={supplierId} className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm">
            <option value="">Te gjithe furnitoret</option>
      ...
      <div className="grid gap-3 md:grid-cols-4">
        ...
        <div className="rounded-xl border bg-white p-4 shadow-sm"><div className="text-xs text-slate-500">Closing Payable</div><div className="text-lg font-semibold">{money(statement.closingBalance)}</div></div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm">
            <thead className="bg-slate-50 border-b">
              <tr>
                {['Data', 'Furnitori', 'Dokumenti', 'Tipi', 'Reference', 'Debit', 'Credit', 'Balance'].map((title) => (
