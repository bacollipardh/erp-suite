  ...
  if (data.length === 0) {
    return (
      <div className="rounded-xl border bg-white">
        ...
        <div className="text-center py-12 text-sm text-slate-400">Nuk ka te dhena.</div>
      </div>
    );
  }
        ...
        <table className="min-w-full text-sm">
          <thead className="bg-slate-50 border-b">
            <tr>
              ...
              {canOpenDetails ? <th className="px-3 py-2.5 w-10" /> : null}
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-100">
            {data.map((row, index) => (
              ...
              >
                ...
                {canOpenDetails && detailsBasePath && row.id ? (
