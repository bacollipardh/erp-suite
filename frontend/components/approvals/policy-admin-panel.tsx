...
export function PolicyAdminPanel({ policies }: { policies: Policy[] }) {
  ...
  function fields(form: FormState, update: (key: keyof FormState, value: string | boolean) => void) {
    return <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
      ...
      <label className="flex items-center gap-2 rounded-xl border bg-slate-50 px-3 py-2 text-sm"><input type="checkbox" checked={form.isActive} onChange={(e) => update('isActive', e.target.checked)} /><span className="font-medium text-slate-700">Active</span></label>
    </div>;
  }
