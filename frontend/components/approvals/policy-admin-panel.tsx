...
type Policy = { id: string; code: string; name: string; entityType: string; action: string; minAmount: number | null; maxAmount: number | null; requiredSteps: number; isActive: boolean };
...
type FormState = { code: string; name: string; entityType: string; action: string; minAmount: string; maxAmount: string; requiredSteps: string; isActive: boolean };
...
const defaults: FormState = { code: 'NEW_POLICY', name: 'New approval policy', entityType: 'supplier-payments', action: 'POST', minAmount: '0', maxAmount: '', requiredSteps: '1', isActive: true };

function toBody(form: FormState) {
  return { code: form.code, name: form.name, entityType: form.entityType, action: form.action, minAmount: form.minAmount === '' ? null : Number(form.minAmount), maxAmount: form.maxAmount === '' ? null : Number(form.maxAmount), requiredSteps: Number(form.requiredSteps || 1), isActive: form.isActive };
}

function fromPolicy(policy: Policy): FormState {
  return { code: policy.code, name: policy.name, entityType: policy.entityType, action: policy.action, minAmount: policy.minAmount === null ? '' : String(policy.minAmount), maxAmount: policy.maxAmount === null ? '' : String(policy.maxAmount), requiredSteps: String(policy.requiredSteps), isActive: policy.isActive };
}
...
export function PolicyAdminPanel({ policies }: { policies: Policy[] }) {
  ...
  return <div className="space-y-4">
    <div className="flex justify-end"><button type="button" onClick={() => setCreateOpen((value) => !value)} className="rounded-xl bg-indigo-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-700">New Policy</button></div>
    {createOpen ? <section className="rounded-2xl border bg-white p-4 shadow-sm"><h2 className="text-sm font-semibold text-slate-900">Create Approval Policy</h2><div className="mt-4">{fields(createForm, updateCreate)}</div><div className="mt-4 flex justify-end gap-2"><button type="button" onClick={() => setCreateOpen(false)} className="rounded-lg border px-3 py-2 text-sm">Cancel</button><button type="button" disabled={busy} onClick={createPolicy} className="rounded-lg bg-indigo-600 px-3 py-2 text-sm font-semibold text-white disabled:opacity-50">Create</button></div></section> : null}
    {error ? <div className="rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">{error}</div> : null}
    <section className="overflow-hidden rounded-2xl border bg-white shadow-sm"><div className="border-b bg-slate-50 px-4 py-3 text-sm font-semibold text-slate-700">Approval Policies</div><div className="overflow-x-auto"><table className="min-w-full text-sm"><thead className="border-b bg-white"><tr>{['Status', 'Code / Name', 'Entity', 'Action', 'Range', 'Steps', 'Actions'].map((title) => <th key={title} className="px-3 py-2 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">{title}</th>)}</tr></thead><tbody className="divide-y divide-slate-100">{policies.map((policy) => <tr key={policy.id} className="align-top hover:bg-slate-50/70"><td className="px-3 py-3"><span className={`rounded-full border px-2 py-1 text-xs font-semibold ${policy.isActive ? 'border-emerald-200 bg-emerald-100 text-emerald-700' : 'border-slate-200 bg-slate-100 text-slate-600'}`}>{policy.isActive ? 'ACTIVE' : 'INACTIVE'}</span></td><td className="px-3 py-3 min-w-72"><div className="font-semibold text-slate-900">{policy.code}</div><div className="text-xs text-slate-500">{policy.name}</div></td><td className="px-3 py-3 whitespace-nowrap">{policy.entityType}</td><td className="px-3 py-3 whitespace-nowrap">{policy.action}</td><td className="px-3 py-3 whitespace-nowrap">{policy.minAmount ?? '0'} - {policy.maxAmount ?? '∞'} EUR</td><td className="px-3 py-3 whitespace-nowrap">{policy.requiredSteps}</td><td className="px-3 py-3 whitespace-nowrap"><div className="flex gap-2"><button type="button" onClick={() => startEdit(policy)} className="rounded-lg border px-3 py-1.5 text-xs font-semibold hover:bg-slate-50">Edit</button><button type="button" onClick={() => openSteps(policy)} className="rounded-lg border border-indigo-200 px-3 py-1.5 text-xs font-semibold text-indigo-700 hover:bg-indigo-50">Steps</button></div></td></tr>)}</tbody></table></div></section>
    {editId ? <section className="rounded-2xl border bg-white p-4 shadow-sm"><h2 className="text-sm font-semibold text-slate-900">Edit Approval Policy</h2><div className="mt-4">{fields(editForm, updateEdit)}</div><div className="mt-4 flex justify-end gap-2"><button type="button" onClick={() => setEditId(null)} className="rounded-lg border px-3 py-2 text-sm">Cancel</button><button type="button" disabled={busy} onClick={savePolicy} className="rounded-lg bg-indigo-600 px-3 py-2 text-sm font-semibold text-white disabled:opacity-50">Save</button></div></section> : null}
    {stepPolicy ? <section className="rounded-2xl border bg-white p-4 shadow-sm"><div className="flex items-start justify-between gap-3"><div><h2 className="text-sm font-semibold text-slate-900">Approver Steps — {stepPolicy.code}</h2><p className="text-xs text-slate-500">Vendos role/user approver për çdo hap. Role code shembull: ADMIN, FINANCE_MANAGER.</p></div><button type="button" onClick={() => setStepPolicy(null)} className="rounded-lg border px-3 py-1.5 text-xs">Close</button></div><div className="mt-4 space-y-3">{stepForms.map((step, index) => <div key={step.stepNo} className="grid gap-3 rounded-xl border bg-slate-50 p-3 md:grid-cols-[90px_1fr_1fr_1fr_120px]"><div className="text-sm font-semibold text-slate-900">Step {step.stepNo}</div><input className={inputClass} value={step.label} onChange={(e) => updateStep(index, 'label', e.target.value)} placeholder="Label" /><input className={inputClass} value={step.approverRoleCode} onChange={(e) => updateStep(index, 'approverRoleCode', e.target.value.toUpperCase())} placeholder="Role code" /><input className={inputClass} value={step.approverUserId} onChange={(e) => updateStep(index, 'approverUserId', e.target.value)} placeholder="User UUID optional" /><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={step.isRequired} onChange={(e) => updateStep(index, 'isRequired', e.target.checked)} />Required</label></div>)}</div><div className="mt-4 flex justify-end gap-2"><button type="button" onClick={() => setStepPolicy(null)} className="rounded-lg border px-3 py-2 text-sm">Cancel</button><button type="button" disabled={busy} onClick={saveSteps} className="rounded-lg bg-indigo-600 px-3 py-2 text-sm font-semibold text-white disabled:opacity-50">Save Steps</button></div></section> : null}
  </div>;
}
